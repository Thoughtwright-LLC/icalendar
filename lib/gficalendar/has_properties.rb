module GFIcalendar
  ...
  module HasProperties
    ...
    def append_custom_property(property_name, value)
      property_name = property_name.downcase
      if value.is_a? GFIcalendar::Value
        custom_properties[property_name] << value
      else
        custom_properties[property_name] << GFIcalendar::Values::Text.new(value)
      end
    end
    ...
    module ClassMethods
      ...
      def default_property_types
        @default_property_types ||= Hash.new { |h,k| GFIcalendar::Values::Text }
      end

      def required_property(prop, klass = GFIcalendar::Values::Text, validator = nil)
        validator ||= ->(component, value) { !value.nil? }
        self.required_properties[prop] = validator
        single_property prop, klass
      end

      def required_multi_property(prop, klass = GFIcalendar::Values::Text, validator = nil)
        validator ||= ->(component, value) { !value.compact.empty? }
        self.required_properties[prop] = validator
        multi_property prop, klass
      end

      def optional_single_property(prop, klass = GFIcalendar::Values::Text)
        single_property prop, klass
      end
      ...
      def optional_property(prop, klass = GFIcalendar::Values::Text, suggested_single = false)
    ...
    def map_property_value(value, klass, multi_valued)
      if value.nil? || value.is_a?(GFIcalendar::Value)
        value
      elsif value.is_a? ::Array
        GFIcalendar::Values::Array.new value, klass, {}, {delimiter: (multi_valued ? ',' : ';')}
      else
        klass.new value
      end
    end
