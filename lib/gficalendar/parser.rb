  ...
  class Parser
    ...
    def initialize(source, strict = false)
      if source.respond_to? :gets
        ...
        msg = 'GFIcalendar::Parser.new must be called with a String or IO object'
        GFIcalendar.fatal msg
    ...
    def parse_property(component, fields = nil)
      ...
      begin
        ...
        if strict?
          GFIcalendar.logger.error "No method \"#{method_name}\" for component #{component}"
          raise nme
        else
          GFIcalendar.logger.warn "No method \"#{method_name}\" for component #{component}. Appending to custom."
    ...
    def wrap_property_value(component, fields)
      ...
      if klass.value_type != 'RECUR' && fields[:value] =~ /(?<!\\)([,;])/
        delimiter = $1
        GFIcalendar::Values::Array.new fields[:value].split(/(?<!\\)[;,]/),
                                     klass,
                                     fields[:params],
                                     delimiter: delimiter
      else
    ...
    rescue GFIcalendar::Values::DateTime::FormatError => fe
      raise fe if strict?
    ...
    def get_wrapper_class(component, fields)
      ...
      if !fields[:params]['value'].nil?
        ...
        unless klass_name.upcase == klass.value_type
          ...
          klass = GFIcalendar::Values.const_get klass_name if GFIcalendar::Values.const_defined?(klass_name)
        end
      end
      klass
    end
    ...
    def parse_component(component)
      while (fields = next_fields)
        if fields[:name] == 'end'
          ...
          GFIcalendar.logger.debug "Adding component #{klass_name}"
          if GFIcalendar.const_defined? klass_name
            component.add_component parse_component(GFIcalendar.const_get(klass_name).new)
          elsif GFIcalendar::Timezone.const_defined? klass_name
            component.add_component parse_component(GFIcalendar::Timezone.const_get(klass_name).new)
          else
    ...
    def parse_fields(input)
      ...
      GFIcalendar.logger.debug "Found fields: #{parts.inspect} with params: #{params.inspect}"
      {
