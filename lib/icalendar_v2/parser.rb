  ...
  class Parser
    ...
    def initialize(source, strict = false)
      if source.respond_to? :gets
        ...
        msg = 'IcalendarV2::Parser.new must be called with a String or IO object'
        IcalendarV2.fatal msg
    ...
    def parse_property(component, fields = nil)
      ...
      begin
        ...
        if strict?
          IcalendarV2.logger.error "No method \"#{method_name}\" for component #{component}"
          raise nme
        else
          IcalendarV2.logger.warn "No method \"#{method_name}\" for component #{component}. Appending to custom."
    ...
    def wrap_property_value(component, fields)
      ...
      if klass.value_type != 'RECUR' && fields[:value] =~ /(?<!\\)([,;])/
        delimiter = $1
        IcalendarV2::Values::Array.new fields[:value].split(/(?<!\\)[;,]/),
                                     klass,
                                     fields[:params],
                                     delimiter: delimiter
      else
    ...
    rescue IcalendarV2::Values::DateTime::FormatError => fe
      raise fe if strict?
    ...
    def get_wrapper_class(component, fields)
      ...
      if !fields[:params]['value'].nil?
        ...
        unless klass_name.upcase == klass.value_type
          ...
          klass = IcalendarV2::Values.const_get klass_name if IcalendarV2::Values.const_defined?(klass_name)
        end
      end
      klass
    end
    ...
    def parse_component(component)
      while (fields = next_fields)
        if fields[:name] == 'end'
          ...
          IcalendarV2.logger.debug "Adding component #{klass_name}"
          if IcalendarV2.const_defined? klass_name
            component.add_component parse_component(IcalendarV2.const_get(klass_name).new)
          elsif IcalendarV2::Timezone.const_defined? klass_name
            component.add_component parse_component(IcalendarV2::Timezone.const_get(klass_name).new)
          else
    ...
    def parse_fields(input)
      ...
      IcalendarV2.logger.debug "Found fields: #{parts.inspect} with params: #{params.inspect}"
      {
